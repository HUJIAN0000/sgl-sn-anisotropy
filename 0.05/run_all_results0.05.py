# -*- coding: utf-8 -*-
"""
run_all_results0.05.py  --  5%-TOLERANCE one-shot reproduction.

Run this INSIDE the 5% sample folder (program + data in the same folder; all file
names are RELATIVE -- no absolute paths). It reproduces every code-derived result
of the paper for the delta d/d = 0.05 sample, AND draws the getdist triangle plots
...
Run once -> produces:
   ...
   Table B.1 (tolerance, 5% row only here)  -> tableB1_tolerance.{csv,tex}
...
REQUIRED LOCAL FILES (produced by match_milp_headless0.05.py), all in THIS folder:
...
run_all_results0.10.py). This script is for 5% only, so Table B.1 fills the 5% row
...
# 5%-tolerance run: program + data live in the SAME folder, RELATIVE names only.
# Files expected here (produced by match_milp_headless0.05.py):
...
# Baseline = 5%, Omega_m=0.30.
EXISTING_BASELINE_CSV = _samp("0.30")[0]
EXISTING_BASELINE_COV = _samp("0.30")[1:]

# Table D.1 (Omega_m robustness) -- all at 5%, from THIS folder.
...
# Table B.1 (tolerance) -- this folder is 5% only; 3%/10% come from their own runs.
EXISTING_TOL_SAMPLES = {"5%": None}
...
TOL_LIST   = [0.05]                 # this folder = 5% only (B.1 5% row)
...
def main():
    ...
    if not RUN_PAIRING and N != 107:
        log(f"  NOTE: baseline N={N} (paper uses 107). Confirm this is the intended sample.")
    ...
    # ----------------------------------------------------------- Table D.1 (Om)
    if RUN_OMEGA:
        banner("PART 6: Table D.1 -- Omega_m robustness (tol=5%)")
        rows = []
        ...
        log("  >>> The Om=0.30 row should match Table 1/B.1 baseline (dBIC ~ 5.6).")
