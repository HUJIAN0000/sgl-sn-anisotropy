# -*- coding: utf-8 -*-
"""
run_all_results0.10.py  --  10%-TOLERANCE one-shot reproduction.

Run this INSIDE the 10% sample folder (program + data in the same folder; all file
names are RELATIVE -- no absolute paths). It reproduces every code-derived result
of the paper for the delta d/d = 0.10 sample, AND draws the getdist triangle plots
...
Run once -> produces:
   ...
   Table B.1 (tolerance, 10% row only here) -> tableB1_tolerance.{csv,tex}
...
REQUIRED LOCAL FILES (produced by match_milp_headless0.1.py), all in THIS folder:
...
run_all_results0.10.py). This script is for 10% only, so Table B.1 fills the 10% row
...
# 10%-tolerance run: program + data live in the SAME folder, RELATIVE names only.
# Files expected here (produced by match_milp_headless0.1.py):
...
# Baseline = 10%, Omega_m=0.30.
EXISTING_BASELINE_CSV = _samp("0.30")[0]
EXISTING_BASELINE_COV = _samp("0.30")[1:]

# Table D.1 (Omega_m robustness) -- all at 10%, from THIS folder.
...
# Table B.1 (tolerance) -- this folder is 10% only; the other rows come from their own runs.
EXISTING_TOL_SAMPLES = {"10%": None}
...
TOL_LIST   = [0.10]                 # this folder = 10% only (B.1 10% row)
...
def main():
    ...
    if not RUN_PAIRING:
        log(f"  baseline N={N} (expected ~117 at 10% tolerance)")
    ...
    # ----------------------------------------------------------- Table D.1 (Om)
    if RUN_OMEGA:
        banner("PART 6: Table D.1 -- Omega_m robustness (tol=10%)")
        rows = []
        ...
        log("  >>> Om=0.30 row should match this folder B.1 baseline (dBIC ~ 7.6).")
